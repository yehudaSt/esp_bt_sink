...
#define LED_PIN 22  // Commonly GPIO2 on many ESP32 boards (can vary)
...
void app_main(void)
{
    ...
    // Try to connect to Bluetooth for 10 times
    // for (int i=0; i<10; i++)
    // {
    //     int res = bluetooth_service_start(&bt_cfg);
    //     if (res == ESP_OK) {
    //         break;
    //     }
    //     else
    ...
    while (1) {
        ...
        if (ret != ESP_OK) {
            if (ret == ESP_ERR_TIMEOUT) {
                // Timeout waiting for events, continue the loop
                continue;
            }
        ...
        /* Only stop the application if we specifically want to stop */
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT && msg.source == (void *) i2s_stream_writer
            && msg.cmd == AEL_MSG_CMD_REPORT_STATUS
            && (((int)msg.data == AEL_STATUS_STATE_STOPPED) || ((int)msg.data == AEL_STATUS_STATE_FINISHED))) {
            if (bluetooth_connected) {
                // Only exit if this is an intentional stop, not a disconnect
